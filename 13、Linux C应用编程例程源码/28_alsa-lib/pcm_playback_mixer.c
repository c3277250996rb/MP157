...
static int snd_mixer_init(void)
{
    ...
    while (elem) {
        ...
        /* 针对开发板出厂系统：CS42L51声卡设备 */
        if(!strcmp("PCM", elem_name)) {	//PCM播放音量
            if (snd_mixer_selem_has_playback_volume(elem)) {//是否是音量控制元素
                ...
                snd_mixer_selem_set_playback_volume_all(elem, (maxvol-minvol)*0.5 + minvol);//设置为50%
                playback_vol_elem = elem;
            }
        }

        if(!strcmp("Analog", elem_name)) {	//模拟信号播放音量
            if (snd_mixer_selem_has_playback_volume(elem)) {//是否是音量控制元素
                snd_mixer_selem_get_playback_volume_range(elem, &minvol, &maxvol);//获取音量可设置范围
                snd_mixer_selem_set_playback_volume_all(elem, (maxvol-minvol)*0.8 + minvol);//设置为80%
            }
        }
